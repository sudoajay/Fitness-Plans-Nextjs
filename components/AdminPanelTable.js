...
function AdminPanelTable({ databaseData, userName }) {
  ...
  useEffect(() => {
    ...
    setDatabaseData(
      databaseData.filter(
        (element) =>
          element.FullName.toLowerCase().indexOf(getSearch.toLowerCase()) !== -1 ||
          element.Email.toLowerCase().indexOf(getSearch.toLowerCase()) !== -1 ||
          element.Product.indexOf(getSearch.toLowerCase()) !== -1 ||
          element.PaymentMethod.toLowerCase().indexOf(getSearch.toLowerCase()) !== -1
      )
    );
  }, [getSearch  ]);
  ...
  return (
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg pl-8 pr-4 pb-8 pt-8">
            ...
            .map((value, index) =>
              ...
              userName == "ADMIN" ? (
                <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-500">
                  ...
                  <td class="px-6 py-4  dark:text-white">{value.FullName}</td>
                  <td class="px-6 py-4  dark:text-white">{value.Age}</td>
                  <td class="px-6 py-4  dark:text-white">{value.Email}</td>
                  <td class="px-6 py-4 dark:text-white">{value.PhoneNumber}</td>
                  <td class="px-6 py-4 dark:text-white">{value.Information}</td>
                  <td class="px-6 py-4 dark:text-white">{value.Created}</td>
                  <td class="px-6 py-4 dark:text-white">{value.Product}</td>
                  <td class="px-6 py-4 dark:text-white">{value.Amount}</td>
                  <td class="px-6 py-4 dark:text-white">{value.PromoCode}</td>
                  <td class="px-6 py-4 dark:text-white">{value.Referralcode}</td>

                  <td class="px-6 py-4 dark:text-white">
                    {value.PaymentMethod}
                  </td>
                  <td class="px-6 py-4 dark:text-white">{value.PaymentID}</td>
                  <td class="px-6 py-4 dark:text-white">{value.OrderID}</td>
                  <td class="px-6 py-4 dark:text-white">{value.AccessToken}</td>
                </tr>
              ) : (
                ...
                <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-500">
                  ...
                  <td class="px-6 py-4 dark:text-white">{value.Created}</td>
                  <td class="px-6 py-4 dark:text-white">{value.Product}</td>
                  <td class="px-6 py-4 dark:text-white">{value.Amount.toLowerCase().includes("rs") ? parseFloat(value.Amount.toLowerCase().replace("rs","").replaceAll(/\s/g,'') / parseInt(process.env.INR_TO_DOLLOR)).toFixed(2) + " $"  : value.Amount}</td>
                  <td class="px-6 py-4 dark:text-white">{value.PromoCode}</td>
                  <td class="px-6 py-4 dark:text-white">{value.Referralcode}</td>
